...
— D1 사전 측정에서 SWT 튜닝이 재탐색을 하고도 **음수**(tune −0.49, holdout −0.55 dB)를
냈다. 파라미터 문제가 아니었다.

**MIT-BIH 신호는 clean 이 아니다.** 이미 기저선 변동과 고주파 성분을 담고 있는데
우리는 그것을 정답으로 두고 있었다. front-end 가 그 성분을 제거하면 정답에서
**멀어지므로** SNR 이 떨어진다. 실측:

| | 잡음 없이 front-end 만 통과했을 때 원본과의 SNR | 10 dB 에서 front-end 의 효과 |
|---|---|---|
| D0 (합성) | 53.24 dB (무해) | **+1.06 dB** |
| D1 (MIT-BIH) | **9.32 dB** | **−3.10 dB** |

그 결과 D1 에서 모든 방법이 한 점에 뭉치고 무처리가 최고가 된다 (10 dB, 4 record):
...
**평가가 아무것도 구분하지 못한다.** 이 상태로 D1 실험을 돌렸다면 "아무 처리도
하지 않는 것이 최선" 이라는 무의미한 결론이 나왔을 것이다.

조치: 과제의 목표를 **"0.5~100 Hz 대역의 ECG 를 복원한다"** 로 명시하고, 참조도
그 대역으로 맞춘다. `x_ref = FE(x_raw)`. 잡음은 여전히 **원본**에 섞고(취득계는
대역제한 전 신호를 본다), EXP-C 의 왜곡 측정도 원본을 입력으로 준다 — 참조를
입력으로 주면 front-end 가 두 번 걸린다. 학습 타깃도 같은 대역으로 맞췄다.
다르면 신경망이 front-end 를 되돌리는 법을 배운다.

**D0 결과는 재실행하지 않아도 된다.** 같은 비교를 D0 에서 하면 차이가 0.02 dB
이하다(위 표의 53.24 dB 가 그 이유다). 측정으로 확인했다.

> **일반화**: 공개 데이터셋의 '깨끗한' 기록을 ground truth 로 쓸 때, 그것이
> **정말로 우리가 복원하려는 신호인지** 확인해야 한다. 아니라면 전처리를 거친
> 방법일수록 손해를 보고, 그 손해는 성능 차이처럼 보인다. 이 함정은 합성
> 데이터에서는 드러나지 않는다 — 거기서는 clean 이 정의상 clean 이기 때문이다.
...
반면 딥러닝은 학습·추론 양쪽에서 raw 를 받고 있었다. 즉 **기저선 변동 제거를 처음부터
학습해야 하는** 불리한 조건이었다. 입력 10 dB 에서 실측:
...
1. **딥러닝이 진 것처럼 보이던 조건의 상당 부분이 front-end 유무였다.**
2. **bw/pli 에서 SWT 의 기여분은 정확히 0 dB 다** (`M_FE` 20.26 = `M04` 20.26).
   "SWT 가 이 조건에서 oracle 에 근접한다" 는 서술을 철회했다. 그 이득은 front-end 가
   낸 것이다. SWT 가 실제로 일하는 곳은 AWGN(1.46 → 7.91)뿐이다.

조치: `ECGDenoiseDataset` 이 학습 시에도 FE 를 적용하도록 하고(창 양쪽에 실제 이웃
구간을 여유로 붙여 필터링한 뒤 가운데만 사용 — 그러지 않으면 4.096 s 창 전체가 HPF
트랜지언트가 된다), 체크포인트에 `frontend` 플래그를 기록해 추론이 학습과 같은 전처리를
쓰도록 했다. 전 모델 재학습 + 전 실험 재실행이 필요하다.

> **일반화**: "모든 방법에 동일 조건" 같은 규약은 **선언이 아니라 측정으로** 확인해야
> 한다. 그래서 이제 모든 결과표에 `M_FE`(front-end 단독)를 필수로 넣는다. 각 방법의
> 이득 중 얼마가 공통 전처리에서 온 것인지 표 안에서 바로 읽히지 않으면,
> 같은 착각이 다시 일어난다.
