# 04. SNR 추정기 교정표 (D1) — ground truth 없는 실측 신호용

> 자동 생성: `python scripts/check_snr_estimator.py --source mitdb`
...
조건: MIT-BIH 실기록 5 기록 × 120 s, 참 SNR ∈ [0, 5, 10, 15, 20] dB, 잡음 7 종.
...
|:--------------|-----------------:|------------------:|------------------:|
| front-end 미적용 |             10.9 |              11.6 |              17.7 |
| front-end 적용  |             11.9 |              12.8 |              15.6 |
...
|:-----------|-----------------:|------------------:|------------------:|-------:|
| awgn       |             -6.1 |              -5.7 |              -0.7 |    0.2 |
| bw         |             -0.7 |               4.2 |              10   |    0.2 |
| em(motion) |             -1.8 |              -0.2 |              10.4 |    0.2 |
| impulse    |             -6.7 |              -5.6 |               9.8 |    0.2 |
| ma(emg)    |             -5   |              -3.8 |               6.4 |    0.2 |
| mixed      |             -3.9 |              -0.8 |               6.7 |    0.2 |
| pli        |             -0.6 |               2.1 |              -1.4 |    0.2 |
...
| noise      |   (A) beat-resid |   (B) half-sample |   (C) isoelectric |   포화비율 |
|:-----------|-----------------:|------------------:|------------------:|-------:|
| awgn       |             -5.8 |              -5.1 |              -1.6 |    0.1 |
| bw         |            -11.9 |              -8.7 |              -3.2 |    0.9 |
| em(motion) |             -5.1 |              -4.6 |               4   |    0.3 |
| impulse    |             -6.2 |              -4.8 |               7.3 |    0   |
| ma(emg)    |             -5   |              -3.1 |               4.3 |    0   |
| mixed      |             -6   |              -4.1 |               4.7 |    0.3 |
| pli        |            -11.1 |              -8.2 |              -6   |    0.8 |
...
|:-----------|-----------:|-------:|-------:|-------:|
| awgn       |      -7.41 |   0.21 |   0.11 |   0.72 |
| bw         |      -2.95 |   0.54 |   0.12 |   0.96 |
| em(motion) |      -3.81 |   0.65 |   0.16 |   0.96 |
| impulse    |      -7.86 |   0.51 |   0.24 |   1    |
| ma(emg)    |      -6.5  |   0.38 |   0.18 |   0.92 |
| mixed      |      -5.62 |   0.44 |   0.13 |   0.92 |
| pli        |      -2.96 |   0.5  |   0.1  |   1    |
...
5. 추정치가 **12 dB** 이상이면 `ceiling_warning` 이 뜬다 (`SNR_CEILING_BY_AXIS["d1"]`).
   생리학적 beat 변동이 바닥을 만들기 때문에 그 이상은 구분되지 않는다.
   "12 dB 이상" 으로만 해석한다. 이 임계값은 **위 '추정기의 천장' 표에서 실측한 값**이고, 축마다 다르다 (F-18).
...
```bash
python scripts/check_snr_estimator.py --source mitdb
```
