...
| **블록 영위상 0.5 s** | -33.46 | **-36.42** | +0.35 | 11.05 |
| **중앙값 200+600 ms** | **-30.56** | -28.93 | -2.20 | 4.58 |

**순위가 뒤집힌다.**

* `FE_off` 기준: 블록 영위상이 **7.49 dB 우세**, 8 기록 **전부** 블록 승.
* 참값 기준: 중앙값이 **2.90 dB 우세**, 8 기록 중 1 기록에서 중앙값 승.
...
2.90 dB 로 작고 기록마다 부호가 갈린다(8 중 1). 그리고
**인과 방식은 어느 기준으로도 확실히 진다**(-22.67 대 -33.46/-30.56).
...
* **준위 산포**: 중앙값 4.58 대 블록 11.05 %R — 중앙값 압승
* **ST 준위 오차**: 중앙값 -2.20 대 블록 +0.35 %R — 블록 압승
...
| 블록 영위상 0.5 s | 보통 (11.1) | **가장 좋음 (+0.4)** | 596 ms | ST 보존 103 % |
...
| 블록 영위상 — `odd` 확장 (전) | 1.231 | 1.067 |
| 블록 영위상 — `constant` 확장 (후) | 0.364 | 0.988 |
| 중앙값 200+600 ms | 0.193 | 0.918 |
| 인과 o1 0.5 Hz | 0.178 | 0.901 |

**블록 영위상만 바닥의 5.9 배였다**(D0). 중앙값과 인과는 바닥과 같다 —
블록화 방식 고유의 결함이다. **교차 페이드는 여기에 거의 영향이 없었다**
(1.32 -> 1.23) — 이음매와 굴곡은 원인이 다르다. 아래에서 고쳐 **1.7 배**로
내렸고, 표의 «후» 행이 그 결과다.
