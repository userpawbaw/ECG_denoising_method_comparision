...
* `FE_off` 기준: 블록 영위상이 **6.05 dB 우세**, 8 기록 **전부** 블록 승.
* 참값 기준: 중앙값이 **0.78 dB 우세**, 8 기록 중 4 기록에서 중앙값 승.
...
0.78 dB 로 작고 기록마다 부호가 갈린다(8 중 4). 그리고
**인과 방식은 어느 기준으로도 확실히 진다**(-22.67 대 -29.78/-30.56).
...
* **준위 산포**: 중앙값 4.58 대 블록 11.18 %R — 중앙값 압승
* **ST 준위 오차**: 중앙값 -2.20 대 블록 -0.20 %R — 블록 압승
...
+4.2 %R 왜곡한다. **평평함만 보면 가장 왜곡이 큰 방식이 1 위가 된다.** 그래서 임상에서 실제로 읽는 양으로 갈랐다 —
**ST 편위는 그 박동 자신의 기준선에 대해 읽는다.** 절대 준위는 뜻이 없다.
...
| 블록 영위상 0.5 s | 보통 (11.2) | **가장 좋음 (-0.2)** | 596 ms | ST 보존 101 % |
