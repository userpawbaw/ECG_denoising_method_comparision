...
`m06_l1` 을 seed 0~9 로 돌린다. F-40 의 sd 를 **df=2 에서 df=9** 로 올린다 —
신뢰구간이 0.39~4.66 dB 에서 약 0.51~1.35 dB 로 좁아진다.
...
독립 표본으로 0.25 dB 를 가르려면 팔당 138 판이 필요하다. 그러나 두 arm 이
**같은 seed 에서 함께 높거나 함께 낮다면**(학습 데이터 순서 · 잡음 뽑기가 공통),
그 공통 성분은 차이를 빼면 사라진다. 필요한 판 수는 `sd(Δ)` 로 다시 계산되고,
상관 ρ 가 높을수록 크게 준다. 분석기가 ρ 와 「짝 이득」을 함께 출력한다.
