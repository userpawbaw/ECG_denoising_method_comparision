...
| `mode` | `garrote` |
| `protect_qrs` | `True` |
| `k` (D1..D5) | `(2.5, 2.0, 0.6, 0.4, 0.3)` |

| 조건 | mean `snr_imp_scaled` [dB] | worst-case [dB] |
|---|---|---|
| 튜닝 세트 | 11.12 | 0.17 |
| **검증 세트(미사용 seed)** | **11.39** | 0.20 |
| 교과서 기본값 (level MAD + soft + k=1) | -3.47 | -8.66 |

## 해석

- 검증 세트에서 11.39 dB vs 교과서 기본값 -3.47 dB → **차이 +14.86 dB**.
