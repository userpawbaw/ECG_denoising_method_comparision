...
**(1) noise 표준편차 추정 — level별로**
```
σ_j = MAD(D_j) / 0.6745          # D_j 는 level j 의 detail 계수
```
`D1` 하나로 전 level을 추정하는 관행이 있지만, ECG는 대역별 noise 특성이 다르므로 **level별 추정**을 기본으로 한다.

**(2) level별 threshold 스케일**
```
λ_j = k_j * σ_j * sqrt(2 * ln(N))
```
`k_j`는 A-3의 대역 표에 따라 다르게 준다. 초기값:
```
k = {D1: 1.0, D2: 1.0, D3: 0.4, D4: 0.4, D5: 0.6}   # D3/D4 = QRS 대역이므로 보수적
A5 는 thresholding 하지 않는다 (baseline은 별도 HPF로 처리)
```
이 `k` 벡터 자체가 **ablation 대상**이다.
