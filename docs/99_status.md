...
### 7.3.1 시연 카드 — **정해야 할 것 셋** (사용자 판단 대기)

`results/metric_cards/*.png` 여섯 장(**docs/32**)과 그것을 웹 카드뉴스로 옮긴
세 디자인 안(**docs/33** · `demo/cards/`)이 있다. 사용자 피드백에서 나온
**아직 반영 안 된 것 셋**이다 — 방향이 정해지면 세 안 중 하나에 맞춰 한 번에 한다.

| | 무엇 | 상태 |
|---|---|---|
| **C1 박동 선정** | 카드가 뽑은 박동이 **T 파가 뒤집힌 기록**에 걸렸다. `jitter_kernel` 이 12 % 확률로 T 를 뒤집는 것(의도된 변이)이라 버그는 아니지만 교육용으로 부적합하다 | `pick_teaching_beat()` 를 넣어 **P 양수 · T 양수 · R 지배적 · PVC 아님 · 자기 템플릿에 가까움**으로 점수화해 D0/D1 양쪽에서 고른다 — **미구현** |
| **C5 재설계** | `halluc_energy`(P1/P2)에서 `M05` 는 중간이라 그림이 일을 안 했다. **P3(PVC 훼손)에 증거가 있다 — 단 D0 에만.** `M05` `V−N` 은 **d0 −0.066 / d1 +0.014**(보고서 §5.5 는 D0, §5.8.4 가 D1). 이 칸에 처음 적을 때 **축을 안 적고 인용해 D1 것으로 잘못 적었다 — F-28** | «3 초 지움» 대신 **D0↔D1 대조**로 바꾼다: 합성에서만 보이던 위험이 실데이터에서 사라진다(F-8 과 같은 이야기). `exp_e` 는 지표만 남기고 **파형을 안 저장**하므로 P3 코드를 양축에서 다시 돌려 뽑아야 한다 — **미구현** |
| **C4 · C6 통합** | 둘 다 «차이가 없다» 를 말해 중복이다. C4 는 *잴 수 없는 것*, C6 은 *실제로 없는 것* | **C4 를 남기고** C6 의 메시지를 한 줄로 흡수. 방법 수를 15 → 7 로 줄이고 «잴 수 있는 쪽 / 없는 쪽» 두 덩어리로 재설계 — **미구현** |

그리고 **디자인 안 B·C 는 아직 레인 렌더러를 안 썼다.** `demo/cards/card_core.js`
...
바꾸기 전에 **`verify_stream_processor.py` 를 다시 돌려야 한다** — F-25 가
"필터를 어디서 도느냐" 만으로 `M06` 이 4.3 dB 움직이는 것을 쟀고, D-19 도
예측이 틀렸다. **재기 전에는 안 바꾼다**(F-27 이 세운 규칙).
