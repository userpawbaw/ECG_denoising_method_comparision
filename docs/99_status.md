...
5.8.7 — D1 고 SNR 에서 L3 가 L1 보다 유의하게 낫다(`M08` +1.03 dB, p=1.1e-03).
주 실험은 L1 로 고정돼 있고, TEST 를 보고 바꾸면 사후 선택이 된다.
다시 재려면 **손실을 사전에 L3 로 선언하고 주 실험 전체를 재실행**해야 한다
(학습 7종 + 실험 전부, 약 6 시간). 결론의 방향은 바뀌지 않고 교차점이
13 → 15~16 dB 로 이동하는 정도이므로, 시간이 남을 때 한다.

### 7.4 문서 정리

- `docs/03_metric_floor.md` 만 축 접미사가 없다(`_d0` 없음). 초기 산출물의
  잔재이고, 이름을 바꾸면 참조 4곳을 함께 고쳐야 한다. 급하지 않다.
- `91_report.md` 8장(요약)이 D0 만 반영하고 있다. 5.8 의 결론을 반영해야 한다.

---

## 8. 세션 인수인계 (컨텍스트 압축 대비)

> 이 절은 **여기 적힌 작업이 끝나면 지운다.** 진행 중인 상태를 대화 컨텍스트가
> 아니라 저장소에 남기기 위한 것이다. (P-8 때 같은 방식으로 6절을 썼다 가 지웠다.)

### 8.1 압축으로 잃는 것 / 잃지 않는 것

| | 무엇 | 어디에 있나 |
|---|---|---|
| **안전** | 코드·설정·문서·기록(F/D/O) | 저장소 |
| **안전** | 튜닝값·교정표·체크포인트·그림 | `results/` (O-13 이후 추적) |
| **안전** | 학습 이력·출처 | `log.csv`, `manifest.json` |
| **안전** | M10 설계 근거와 TCN 판단 | `ecgdn/models/dilated_resnet1d.py` 독스트링, 커밋 `1416c86` |
| **잃음** | 백그라운드 작업 ID (`b8ij2evys`) | → 8.5 로 대체 |
| **잃음** | 아래 8.2~8.4 의 내용 | → **이 절이 그 대체물이다** |

**프로세스는 압축으로 죽지 않는다.** 이 세션에서 학습을 죽인 것은 세 번 다
컨테이너 재시작이었다(O-2). 다만 작업 ID 를 잃으므로 완료 알림이 오지 않는다.

### 8.2 M10 (dilated ResNet) — D1 판정 확정, D0 대기

**질문**: U-Net 의 downsample→upsample 이 QRS 를 뭉개는가.
(TCN 을 "구조 하나 더" 로 넣는 대신 좁힌 판 — 근거는 커밋 `1416c86`.)

M10 은 M06 과 params(+0.4 %)·수용영역(+0.9 %)·손실·데이터·일정을 맞췄고
**차이는 시간 해상도 하나**다. 대가로 추론이 **2.25 배** 비싸다.

**D1 TEST, `M10 − M06`, paired Wilcoxon + Holm, 22 기록** `[측정]`:

| metric | Δ | p(Holm) | effect r | 판정 |
|---|---|---|---|---|
| `snr_imp_scaled` | −0.591 | 0.211 | −0.526 | n.s. |
| **`qrs_dur_err_ms`** | **−0.072** | **0.992** | −0.249 | **n.s. ← 핵심 가설** |
| **`beat_cc`** | **−0.0019** | **0.028** | −0.715 | **M10 열등** |
| `gain_bias` (→1) | +0.019 | 4.0e-05 | +0.953 | **M10 열등** (1.029 vs 1.010) |
| `rmse` | +0.0031 | 0.217 | +0.470 | n.s. |
| `psd_logdist` | −0.731 | 0.012 | −0.755 | M10 우수 |

**핵심 가설이 기각됐다.** QRS 폭 오차가 그대로고(−0.07 ms, p=0.99), 이 축에서
실제로 쓸 수 있는 형태 지표인 `beat_cc` 는 **오히려 M10 이 나쁘다.**

> `qrs_dur_err_ms` 는 D1 분해능이 28.07 ms 라 애초에 방법을 가르지 못한다
> (F-16). 그래서 `beat_cc` 가 판정 근거다.

**남은 것**: D0 실험이 진행 중이다(8.5). D0 에서도 같은 결론이면
**"구조는 이 파이프라인의 병목이 아니다" 의 세 번째 증거**가 된다
(앞의 둘: M08 vs M06 D1 n.s., 그리고 이번 M10).

**주의 — VAL 로 판단하지 말 것.** M10 D0 는 운 좋은 6번째 epoch 때문에
18 epoch 에서 조기 종료됐고 M06 D0 는 37 epoch 을 돌았다. VAL `best` 는
런마다 다르게 부풀려진다(F-19). **판정은 TEST 다.**

### 8.3 손실함수 확장 제안 — L5 · L6 (아직 결정 안 됨)

**왜 손실인가**: 구조 변경보다 효과가 크다는 것이 두 번 측정됐다.

| 무엇을 바꿨나 | D1 효과 | p(Holm) |
|---|---|---|
| 구조: M06 → M08 (wavelet subband 입력) | +0.292 dB | 0.70 n.s. |
| 구조: M06 → M10 (downsampling 제거) | −0.591 dB | 0.21 n.s. |
| **손실: M08-L1 → M08-L3** | **+1.032 dB** | **1.1e-03 유의** |

**겨냥할 약점** — EXP-C(잡음 없는 clean 을 통과시켰을 때의 출력 SNR):

| `M04` SWT | `M01` | `M08` | `M06` | `M07` |
|---|---|---|---|---|
| **40.14 dB** | 27.79 | 23.51 | 22.22 | 20.07 |

**딥러닝이 clean 신호를 SWT 보다 17 dB 더 망가뜨린다.** 현재 어떤 손실도
이걸 겨냥하지 않는다. 그리고 그 약점이 고 SNR 에서 드러난다 — 20 dB 에서
`M_FE`(16.543)가 모든 DL 변형(최고 15.421)을 이긴다.

L3 가 효과를 낸 이유도 같은 자리다. 이득이 SNR 에 따라 커진다:
**+0.54 / +0.78 / +1.78** (0 / 10 / 20 dB).

**제안 1 — `L6 = L3 + clean 보존 항`** (더 추천)

residual 구조라 모델 출력이 곧 예측 잡음 `n̂` 이다. **입력이 이미 깨끗하면
`n̂ = 0` 이 정답**이므로, 배치 일부에 clean 을 넣고 `‖n̂‖²` 을 벌점으로 준다.
EXP-C 가 이미 그 효과를 재는 지표로 존재한다. 비용 약 +25 %(추가 forward).

**제안 2 — `L5 = L3 + SNR 균형 가중`** (더 쌈)

MSE 는 오차가 큰 저 SNR window 에 압도적으로 무겁게 실린다 — D1 −5 dB 의
rmse 가 20 dB 의 **12 배**(`docs/92_axis_gap.md`). 모델이 어려운 구간으로
학습되고 쉬운 구간에 적용된다. 각 window 손실을 그 window 의 잡음 전력으로
정규화하면 모든 SNR 이 같은 무게로 들어간다. 추가 forward 없음.

**곁들일 것** — `diff_loss(order=2)`. 지금 1차 차분인데 2차(곡률)로 바꾸면
평활에 더 민감하다. `ecgdn/models/losses.py` 한 줄 변경.

**제안하지 않은 것과 이유**

| 후보 | 왜 |
|---|---|
| SI-SDR (척도 불변) | **이득 보존을 포기한다.** `gain_bias` 는 DL 이 `M_FE` 대비 22 기록 전부에서 이기는(r=1.000) 몇 안 되는 항목이다 |
| 다해상도 STFT | `L4` 의 subband 항이 이미 웨이블릿 도메인 판이고 D1 에서 L3 를 못 넘었다 (+0.993 vs +1.032) |
| 시간영역 QRS 가중 | 같은 이유 — L4 가 QRS 대역(D3/D4)에 2배 가중을 이미 줬다 |
| 적대적 / perceptual | **반대 방향.** 안전성 프로브에서 DL 이 가장 안전한 이유가 residual 구조인데 생성적 압력은 그 강점을 깎는다 |

**목표는 +1 dB 가 아니다.** 5.8.7 에서 L3 만으로도 교차점이 13 → 15~16 dB 로
올라간다고 추정했다. 그 13 dB 가 **장비에서 딥러닝을 쓸지 가르는 숫자**다.

**제약** (착수 전 확인할 것): 새 손실은 양축에서 같은 조건으로 학습해
...
2. **L5 / L6 을 진행할 것인가.** 8.3 참조. **사용자 판단 대기.**
...
# 3) 끝났으면: D0 결과로 8.2 를 완성하고 산출물을 커밋
python3 scripts/analyze_axis_gap.py
git add results/ docs/ && git commit && git push
```

**끝난 뒤 할 일**

- `91_report.md` 5.8 에 M10 결과를 넣는다 (EXP-D/RQ4 가 "wavelet 을 어디에
  둘 것인가" 인데, M10 은 "해상도를 유지할 것인가" 라 **새 절이 맞다**).
- M10 결과를 F 로 승격할지 판단한다. 지금 근거로는 **"구조는 병목이
  아니다" 의 세 번째 증거**이고, 단독 F 보다 기존 서술 보강이 적절해 보인다.
- 이 8절을 지운다.
