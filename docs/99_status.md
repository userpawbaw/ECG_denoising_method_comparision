...
| run | model | loss | D0 best VAL | **D1 best VAL** | D1 best ep | D1 epochs |
|---|---|---|---|---|---|---|
| m06_l1 | resunet1d | L1 | +6.214 | **+3.437** | 34 | 46 |
| m06_l2 | resunet1d | L2 | +7.027 | **+3.921** | 22 | 34 |
| m06_l3 | resunet1d | L3 | +7.081 | **+4.426** | 22 | 34 |
| m07_l1 | resunet1d (SWT 선행) | L1 | +2.628 | **+1.590** | 26 | 38 |
| m08_l1 | wavelet_unet | L1 | +6.737 | **+3.236** | 26 | 38 |
| m08_l3 | wavelet_unet | L3 | +6.769 | **+4.523** | 31 | 43 |
| m08_l4 | wavelet_unet | L4 | +6.843 | **+4.354** | 33 | 45 |

7종 전부 `frontend=True` 로 학습됐다 — 비교 조건이 성립한다. [측정]

**체크포인트의 git hash 가 셋으로 갈린다** (`b497637`, `71502b7`, `6aa1cb3`).
표를 읽는 사람이 F-9 를 떠올릴 조건이라 미리 밝혀 둔다: 그 사이 커밋은
문서와 실험 게이트뿐이고 **학습 경로는 한 글자도 바뀌지 않았다.** 확인 명령:

```bash
git diff --stat b497637..6aa1cb3 -- ecgdn/ configs/ scripts/train.py   # 빈 출력
```

`make_ablation_table.py` 의 거부 조건은 `frontend` 플래그이고 git hash 는 표에
싣기만 한다. hash 차이만으로 거부하면 이런 문서 커밋에도 비교가 막히므로
설계가 맞다 — 다만 **hash 가 갈렸을 때 학습 경로 diff 를 확인하는 것은
사람의 몫**이다. 이번에는 확인했고 동일했다.

### 6.1b `[미확정]` D1 에서 L3/L4 가 L1 을 크게 앞선다

| model | L1 | L2 | L3 | L4 |
|---|---|---|---|---|
| m06 (resunet1d) | +3.437 | +3.921 | **+4.426** | — |
| m08 (wavelet_unet) | +3.236 | — | **+4.523** | +4.354 |

D0 에서는 L1 과 L3 의 차이가 작았는데(m06: 6.214 → 7.081, m08: 6.737 → 6.769)
D1 에서는 m08 기준 **+1.29 dB** 로 벌어진다.

**주의**: 이 값은 VAL 이고, VAL 은 모델 선택에 쓴 지표다. 주 실험(exp_a/b/c)은
L1 체크포인트를 쓰므로, L3 가 정말 낫다면 **주 표가 딥러닝을 과소평가하는 것이
된다.** 다만 지금 config 를 L3 로 바꾸는 것은 VAL 을 보고 고르는 사후 선택이다.
`abl_loss` 의 TEST 결과를 보고 판단한다 — 그것이 이 ablation 을 둔 이유다.

### 6.2 `[미확정]` D1 VAL 이 D0 의 약 6할이다

앞서 2종만 보고 **"절반"** 이라고 적었는데, 7종을 다 받고 보니 정확히는
**0.48~0.67 (평균 0.59)** 이다. 절반이라는 표현은 마침 먼저 끝난 두 런이
비율이 가장 낮은 축에 속해서 나온 것이다. [측정]

| run | D0 | D1 | 비율 |
|---|---|---|---|
| m08_l1 | +6.737 | +3.236 | 0.480 |
| m06_l1 | +6.214 | +3.437 | 0.553 |
| m06_l2 | +7.027 | +3.921 | 0.558 |
| m07_l1 | +2.628 | +1.590 | 0.605 |
| m06_l3 | +7.081 | +4.426 | 0.625 |
| m08_l4 | +6.843 | +4.354 | 0.636 |
| m08_l3 | +6.769 | +4.523 | 0.668 |

**모델 구조와 무관하게 일관된 감소**다. `m07_l1` 은 절대값이 가장 낮은데도
(D0 +2.628) 비율은 중간(0.605)이라, 감소가 특정 구조의 문제가 아님을 보인다.

비율이 loss 에 따라 갈린다: **L1 런 평균 0.546, L3/L4 런 평균 0.643.**
즉 D1 에서 손해를 특히 크게 보는 것은 L1 이다 (6.1b 와 같은 현상의 다른 표현).

**아직 F 로 승격하지 않는다.** 이 값은 front-end 출력 대비 딥러닝의 순수
추가 기여이고, 두 해석이 모두 살아 있다.
