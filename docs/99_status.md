...
### 8.2 M10 (dilated ResNet) — **완료.** 본문으로 옮겼다

**결과는 `91_report.md` 5.9 (RQ7), 전문은 F-20, 설계 결정은 D-11 에 있다.**
D0·D1 양축 확정: 핵심 가설 기각, `beat_cc`·`gain_bias` 는 양축에서 M10 열세,
연산량 10.4 배. `M10` 은 비교군으로 남기고 후보에서 뺀다.

> **이 절에 처음 적었던 D1 표에 오류가 둘 있었다** — 옮기기 전에 잡았다.
>
> 1. `psd_logdist` −0.731(p = 0.012)을 "M10 우수" 로 적었다. D1 floor 가
>    0.998 이라 **분해능 아래**고, D0 에서는 **부호가 반대**다. → O-14,
>    그리고 `compare_methods(floor=...)` 로 관문을 코드에 넣었다.
> 2. 추론 비용 "2.25 배" 는 근거가 없었다. 실측 **3.0 배**(CPU 1 thread),
>    이론 연산량 **10.4 배**다.
...
- [x] `91_report.md` 5.9 에 M10 결과를 넣었다 (RQ7 로 등록. RQ6 은 이미
      장비 전이라 번호가 겹쳤다). F-20 · D-11 · O-14 를 남겼다.
- [ ] **D0 `exp_b` · `abl_loss` · `exp_e` 가 아직 안 끝났다.** 끝나면
      `90_results_d0.md` 를 재생성하고 5.9.5 표의 D0 손실 수치를 갱신한다
      (지금 실린 D0 abl_loss 값은 `M10` 이전 산출물이다 — 5.9.5 에 명시해 뒀다).
- [ ] `python3 scripts/analyze_axis_gap.py` 재실행 후 `92_axis_gap.md` 갱신.
- [ ] 이 8절을 지운다 (8.3 L5/L6 과 8.4 미결 3건만 남으면 그때).
