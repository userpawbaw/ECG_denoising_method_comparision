...
합성축에서는 T-P 를 훨씬 평평하게 만들지만(0.6 vs 7.6 %R) **실기록에서는 그 이점이
사라지고**(6.8 vs 7.2 %R) S 오차만 남는다(4.5 vs 0.0 %R). 무엇보다 **비선형이라
중첩이 깨진다**(잔차 −17.6 dB, 블록 영위상은 −257 dB) — 우리 평가는 참조를
`FE(clean)` 으로 두므로(D-3) front-end 가 선형이라는 가정 위에 서 있다.
