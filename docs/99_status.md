...
| **C5 재설계** | `halluc_energy`(P1/P2)에서 `M05` 는 중간이라 그림이 일을 안 했다. **P3(PVC 훼손)에 증거가 이미 있다** — `M05` 만 `V−N = −0.0645`, 나머지는 양수(보고서 5.5). 기제도 있다: 정상 beat 로 만든 위상 템플릿이 PVC 를 정상 쪽으로 끌어당긴다 | «3 초 지움» 대신 **PVC 박동 하나 확대**로 바꾼다. `exp_e` 는 지표만 남기고 **파형을 안 저장**하므로 P3 코드를 다시 돌려 뽑아야 한다 — **미구현** |
