...
| **R-4** | **스트리밍 처리기** | `ecgdn/realtime/` — 배관 손해는 24 설정 전부 **1.13 dB 이내**. 그런데 **영위상 front-end 는 실시간이 불가능**하고(링잉 64 s), 인과 FE 로 바꾸면 **학습된 모델만** 잃는다: `M06-L1` −4.3 dB vs **`M06-L6` −0.5 dB** (2.4 · **F-25**) |
