...
> P-9(M10) 은 끝나서 본문으로 옮겼다: `91_report.md` 5.9(RQ7), F-20, D-11, O-14.
> 압축 인수인계용으로 썼던 8.1(잃는 것 분류)·8.2(M10 중간 결과)·8.5(재개 절차)는
> 그 일이 끝나 지웠다. **여기 남은 둘은 착수 여부가 사용자 판단에 달려 있다.**

### 8.1 손실함수 확장 제안 — L5 · L6

**왜 손실인가**: 구조 변경보다 효과가 크다는 것이 두 번 측정됐다.

| 무엇을 바꿨나 | 효과 | p(Holm) |
|---|---|---|
| 구조: M06 → M08 (wavelet subband, D1) | +0.292 dB | 0.70 n.s. |
| 구조: M06 → M10 (downsampling 제거, D1) | −0.591 dB | 0.21 n.s. |
| 구조: M06 → M10 (downsampling 제거, D0) | −0.912 dB | 0.02 **열세** |
| **손실: M08-L1 → M08-L3 (D1)** | **+1.032 dB** | **1.1e-03 유의** |
| **손실: M06-L1 → M06-L3 (D0)** | **+1.175 dB** | **1.7e-04 유의** |

구조 변경은 세 번 다 개선이 없었고(5.9.7), 손실 변경은 **양축 모두** 유의한
것이 하나씩 있다.

**겨냥할 약점** — EXP-C(잡음 없는 clean 을 통과시켰을 때의 출력 SNR):

| `M04` SWT | `M01` | `M08` | `M06` | `M10` | `M07` |
|---|---|---|---|---|---|
| **40.14 dB** | 27.82 | 23.98 | 22.43 | 20.92 | 20.21 |

**딥러닝이 clean 신호를 SWT 보다 16~20 dB 더 망가뜨린다.** 현재 어떤 손실도
