...
### 7.1 O-10 — 탐색 산출물이 재실행으로 덮인다

`results/{tag}/tune_swt/` 가 실행별로 나뉘지 않아 재실행하면 이전 근거가
사라진다. D1 실험이 끝났으므로 이제 경로를 바꿔도 안전하다.
`tune_swt/{git_hash}/` 로 나누고 최신을 심볼릭 링크로 가리키는 방식을 검토한다.

O-11 에서 제안한 **산출물 신선도 검사**도 함께 넣는다 — 산출물에 생성 시점
git hash 를 적고, 그 이후 생성 경로가 바뀌었으면 경고한다. `ablation_loss` 는
체크포인트에 대해 이미 하고 있고, floor·튜닝 산출물에는 없다.
