...
### 7.2 F-13 — SNR 추정기 교정이 P-10 의 선행 조건이다

5.8.2 가 **입력 SNR 13 dB** 를 딥러닝 사용 여부의 분기점으로 만들었다. 그런데
실측 장비가 그 위인지 아래인지 판단하려면 ground-truth 없는 SNR 추정기가
필요하고, 그 추정기는 실데이터에서 8 배 편향된다(F-13).

**즉 판단 기준은 정해졌는데 측정 수단이 없다.** Arduino 수집(STEP 28)보다
추정기 교정(STEP 29의 일부)이 먼저다. P-8 전에는 이 의존성이 없었다.
