...
### 3.2 학습 체크포인트 — **어느 것도 최종본이 아니다**

| 위치 | 소스 | front-end | loss | 상태 |
|---|---|---|---|---|
| `results/d0_synth/m06_l1` | synthetic | ✅ | L1 | 완료 (best ep 13, **+6.214 dB**) |
| `results/d0_synth/m08_l1` | synthetic | ✅ | L1 | 완료 (best ep 15, **+6.737 dB**) |
| `results/d0_synth/m07_l1` | synthetic | ✅ | L1 | **중단** (ep 8 에서 SIGKILL) |
| `results/d0_synth/m06_l3` | synthetic | ❌ 없음 | L3 | **무효** (FE 수정 전 학습) |
| `results/d0_synth/m08_l4` | synthetic | ❌ 없음 | L4 | **무효** (FE 수정 전 학습) |
| `results/m06_l1` | **mitdb** | ✅ | L1 | **불완전** (컨테이너 재시작으로 중단) |
| m06_l2, m08_l3 | — | — | — | 학습된 적 없음 |

**D0 학습이 완료된 적이 없다.** m07_l1 중단 + 4개 미완.

`snr_imp` 값 해석 주의: front-end 적용 후에는 모델 입력이 이미 FE 출력이므로,
이 값은 **front-end 가 한 일을 뺀 딥러닝의 순수 추가 기여**다. FE 없던 시절의
`m08_l1 +12.11 dB` 와 직접 비교하면 안 된다 (`train.py:40` 의 `snr_in` 이
모델 입력 기준).
...
| ID | 문제 | 영향 |
|---|---|---|
| **M-1** | `source: auto` 가 작업 디렉터리 상태에 따라 D0/D1 을 조용히 바꾼다 | 재현성. 같은 커밋+config 가 다른 결과를 낸다 |
| **M-2** | 보조 스크립트 4개가 `synth_ecg` 하드코딩 | D1 로 재실행할 수단이 없다. 특히 SWT 튜닝은 TRAIN split 재탐색이 설계상 필수 |
| **M-3** | D0 학습 미완 (m07_l1 중단, 4개 미실행) | loss ablation(STEP 19) DoD 표를 만들 수 없다 |
| **M-4** | 장시간 백그라운드 작업이 turn 종료·컨테이너 재시작에서 죽는다 | 학습이 2회 중단됐다. 재개 수단이 없어 매번 처음부터 |
| **M-5** | D0 실험 결과 전부 무효 (FE 수정 전) | 보고서 5장 수치가 잠정값 |
...
→ 학습에 **체크포인트 재개(resume)** 가 없다. `last.pt` 는 저장되지만
그로부터 이어서 학습하는 경로가 `train.py` 에 없다. `[불확실]` — 재개 기능이
어느 정도 규모의 수정인지는 아직 보지 않았다.
...
| 순서 | 작업 | 완료 조건 |
|---|---|---|
| **P-1** | M-1 해결: `--source` 명시 오버라이드 + 결과 경로를 소스별 분리 | 같은 커밋+config+source 가 항상 같은 결과 |
