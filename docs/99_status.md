...
| **P-8** | D1 학습 + 실험 + 보고 | D0/D1 비교표. **진행 중 — 아래 6절 참조** |
...
## 6. P-8 진행 상황 (D1 학습·실험)

> 이 절은 **작업이 끝나면 지운다.** 진행 중인 상태를 컨텍스트가 아니라 저장소에
> 남기기 위한 것이다.

### 6.1 D1 학습 — 2/7 완료

| run | loss | best VAL | best epoch | 상태 |
|---|---|---|---|---|
...
| run | D0 | D1 | 비율 |
|---|---|---|---|
| m06_l1 | +6.214 | +3.437 | 0.55 |
| m08_l1 | +6.737 | +3.236 | 0.48 |

두 모델 모두 일관되게 절반이다. **아직 F 로 승격하지 않는다.**

이유: 이 값은 **front-end 출력 대비 딥러닝의 순수 추가 기여**다. 따라서 두 가지
해석이 모두 가능하다.

1. 실데이터에서 딥러닝이 실제로 약하다 (F-8 의 연장 — 합성이 DL 에 유리했다)
2. front-end 가 실데이터에서 더 많은 일을 해버려 **남은 여지가 작다**
   (그렇다면 자연스러운 결과이고 딥러닝의 문제가 아니다)

**가르는 방법**: TEST split 에서 `M_FE` 기준선과 함께 본다. 2번이 맞다면
`M_FE` 자체의 개선폭이 D1 에서 더 클 것이고, `M_FE` 대비 딥러닝의 상대적 위치는
D0 와 크게 다르지 않을 것이다. 1번이 맞다면 `M_FE` 대비로 봐도 딥러닝이 밀린다.

또 하나 확인할 것: D0 에서 `M06`/`M08` 이 `M01` 대비 **22개 record 전부**를
이겼는데(effect r ≈ 1.0), D1 에서도 유지되는지.
...
1. 6.2 의 두 해석 중 어느 쪽인가
2. **F-14 의 잠정 딱지를 뗄 수 있는가** — D0 에서 `M05`(Sameni)는 `M01` 과
   통계적으로 구분되지 않았다(p=0.83). D1 에서도 그렇다면 "Sameni 는 자기 생성
   모델 위에서만 강하다" 가 확정된다
3. D0 의 주요 결론(15 dB 에서 `M08` 최고, 임펄스에서 DL 이 oracle 초과,
   wavelet 이 bw/em 에서 기여 0)이 D1 에서 재현되는가

### 6.4 P-8 이후 처리할 것

- **O-10**: 탐색 산출물이 재실행으로 덮어써지는 문제. 실행별 디렉터리
  (`tune_swt/{git_hash}/`)를 두면 해결되지만, 지금 경로를 바꾸면 진행 중인
  D1 결과와 어긋나므로 P-8 이 끝난 뒤 적용한다.
- `91_report.md` 에 D0/D1 비교 장 추가.
- 이 6절 삭제.
