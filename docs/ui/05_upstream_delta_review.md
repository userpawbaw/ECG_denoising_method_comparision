# 외부 UI/UX 시스템 **2 차** 검토 — `62b65b1` → `08ffec3` 의 변경분 `[검토 중]`
...
> 원본 = 방법론 계보(upstream). 이 저장소 = 이 프로젝트의 운영 원본. 원본이 이후
...
| 원본이 검토한 판 | 매니페스트 **0.6.0** (`f9f05cd`, 2026-08-21) — **버전이 두 배 넘게 뛰었다.** 원본 CASE-003 이 「@latest 라 결과가 달라진다, 버전을 적어라」고 스스로 경고한 그 상황 |
