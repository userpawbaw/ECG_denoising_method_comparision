...
| | **`playground`** (Anthropic, 공식 마켓플레이스 `./plugins/playground`) | **후보 — 미결(UD-13 열린 결정 2)** | 2026-09-20 `[측정]` · manifest·SKILL.md 확인 | 스킬이 요구하는 구조가 `layout_b` 와 **똑같다** — 단일 HTML · 인라인 · `state` 객체 · live preview · **네트워크 불필요**(403 과 무관). 쓸 자리: 3 m 가독(§10)과 레인 기하를 손으로 미는 탐색기 |
...
| | **Superdesign** (스킬 `superdesigndev/superdesign-skill` sha `f9f05cd9` · CLI `@superdesign/cli` **0.14.0**) | **자료 = 채택(UD-13, 오늘부터) · 도구 = 로컬 전용, 계정 대기(UD-12)** | 2026-09-20 `[측정]` · `npm pack` + GitHub 문서 9 개 `http=200` | **둘을 나눠서 본다.** ① **자료**: 방법 문서 8 항을 `06_superdesign_method_review.md` 로 옮겨 채택 — 네트워크·계정 불필요. ② **도구**: `search-prompts`·`extract-website`·`create-project --template`·`iterate-design-draft --mode branch`. **`api.superdesign.dev` 가 `403 to CONNECT`** → 로컬에서만. **안 쓰는 것**: `design-system.md` 를 원본으로 · `confirm-generation`(크레딧) · 프레젠테이션·그래픽. 로컬 실행 시 `DO_NOT_TRACK=1`. **공식 마켓플레이스에 플러그인으로 있다**(UF-11) — 켤지는 사용자 결정 |
...
| `dual-creative-director` | 발산 둘의 독립 — 순서·격리·**VS 줄세우기**·사용자 정렬·교차 검토·가지치기 | 외부 `ecg-gui-design-review` **핸드오프** `handoffs/DUAL_CREATIVE_DIRECTOR_IMPLEMENTATION_2026-09-18.md`(`08ffec3`) — **원본에도 스킬은 아직 없다**, 여기가 첫 구현 | **B 는 Superdesign(로컬), A 는 레퍼런스 카드**(UD-12 — 초판의 `frontend-design` 대체는 라운드 ① 이 반증했다) · 모델 분리 → **환경 분리**(클라우드 B / 로컬 A, UF-8) · 산출물은 `results/screens/refs/round<N>/` 한 폴더 · **검증기 앞에 VS + 사용자 정렬**(UO-5 · `02_benchmark` §11) · 브랜치 파일명 ↔ UD 검사 |
