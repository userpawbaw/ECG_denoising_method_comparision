...
| 옵션 | 값 | 기본 | 설명 |
|---|---|---|---|
| `--stage` | `blocks` · `calib` · `capacity` · `cond` · `depth` · `l6cond` · `loss` · `overlap` · `spread` · `structure` | — | 미리 정의된 arm 묶음. --arms 와 함께 쓰면 --arms 가 이긴다 |
...
| `--no-resume` | **켬/끔** | 끔 | 끊긴 판을 처음부터 다시 돌린다 (기본은 last.pt 에서 재개) |
| `--keep-ckpt` | **켬/끔** | 끔 | best.pt/last.pt 를 남긴다 (업로드가 커진다) |
| `--cond-offsets` | 문자열 | — | 교란 격자를 직접 준다 [dB] (기본 [-5, -2, 2, 5]). F-13 의 추정기 편향(+7.6)처럼 기본 격자 밖을 재야 할 때 쓴다 |
| `--cond-perturb` | **켬/끔** | 끔 | 조건화 모델을 **틀린 조건값**으로도 평가한다 (참값 [-5, -2, 2, 5] dB). 학습은 다시 안 한다 — 같은 가중치를 다시 잴 뿐이다 (D-28 4 번) |
| `--dry-run` | **켬/끔** | 끔 | 무엇을 돌릴지만 출력 |
| `--hf-repo` | 문자열 | — | Hugging Face **데이터셋** 저장소로 동기화한다. 시작할 때 받아 오고(끊긴 판이 이어진다), N epoch 마다 last.pt 를 올리고, 판·sweep 이 끝나면 전부 올린다. 토큰은 huggingface-cli login 또는 HF_TOKEN 에서 찾는다 |
| `--hf-every` | 정수 | `10` | 몇 epoch 마다 last.pt 를 올릴지 (기본 10). 1 로 두면 저장소가 빨리 커진다 — HF 는 git 이라 이력이 쌓인다 |
| `--hf-public` | **켬/끔** | 끔 | 저장소를 공개로 만든다 (기본은 비공개). **실측 생체신호를 올릴 때는 쓰지 마라** |
...
| 옵션 | 값 | 기본 | 설명 |
|---|---|---|---|
| `--check` | **켬/끔** | 끔 | 생성하지 않고 **최신인지만** 확인한다 (어긋나면 종료 1) |
...
| 옵션 | 값 | 기본 | 설명 |
|---|---|---|---|
| `--only` | 문자열 | — | 화면 이름 일부 (기본: 전부) |
| `--width` | 정수 | — | 폭 (여러 번 줄 수 있다). 기본 1920 |
| `--theme` | `light` · `dark` · `both` | `light` | 어느 테마로 찍을지. both 는 화면당 두 장 — 다크는 자동 반전이 아니라 따로 고른 값이라 둘 다 봐야 한다 (UF-5) |
| `--all` | **켬/끔** | 끔 | 세 폭 × 두 테마 |
| `--full-page` | **켬/끔** | 끔 | 스크롤 전체를 찍는다 |
...
| 옵션 | 값 | 기본 | 설명 |
|---|---|---|---|
| `--mode` | `dark` · `light` | `light` | 검사 기준이 되는 표면 모드. 명도대·기본 표면색이 모드마다 다르다 (--surface 로 표면만 따로 줄 수 있다) |
| `--surface` | 문자열 | — | 차트 표면색 (기본은 모드별) |
| `--scope` | `lane` · `legend` | `lane` | lane=모든 쌍(엄격) · legend=인접 쌍 (docs/ui 01_system 2.1) |
| `--json` | **켬/끔** | 끔 | 사람이 읽는 표 대신 JSON 으로 낸다 — 검사·스크립트가 쓴다 (tests/test_palette.py) |
