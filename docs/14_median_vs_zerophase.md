...
## 왜 중앙값이 중심선에 «붙어» 보이는가 — **«0» 의 정의가 다르다** `[측정]`
...
**d0 기록 S022** — 전체 평균은 셋 다 0 근처인데 T-P 준위는 갈린다:

| 방식 | 전체 평균 | **T-P 준위** | TP 비중 | 역산값 |
|---|---:|---:|---:|---:|
| 오프라인 영위상 | -0.68 %R | **-8.42 %R** | 29 % | -8.42 %R |
| 블록 영위상 0.5 s | -0.70 %R | **-8.95 %R** | 29 % | -8.95 %R |
| 중앙값 200+600 ms | +7.77 %R | **-0.12 %R** | 29 % | -0.12 %R |

**d1 기록 100** — 전체 평균은 셋 다 0 근처인데 T-P 준위는 갈린다:

| 방식 | 전체 평균 | **T-P 준위** | TP 비중 | 역산값 |
|---|---:|---:|---:|---:|
| 오프라인 영위상 | -0.05 %R | **+0.33 %R** | 30 % | +0.33 %R |
| 블록 영위상 0.5 s | +0.01 %R | **+0.59 %R** | 30 % | +0.59 %R |
| 중앙값 200+600 ms | +2.18 %R | **+0.65 %R** | 30 % | +0.65 %R |

**역산값이 실측과 일치한다** — 위 설명이 성립한다는 검산이다.
...
**d0 기록 S022** · 기저선 변동 50 %R 주입

| | S 깊이 | **S 반치폭** |
|---|---:|---:|
| 참값 (변동 없는 원본) | 21.0 %R | **20 ms** |
| 아무것도 안 함 | 19.8 %R | **16 ms** |
| 오프라인 영위상 | 20.9 %R | **20 ms** |
| 블록 영위상 0.5 s | 20.4 %R | **20 ms** |
| 중앙값 200+600 ms | 23.8 %R | **20 ms** |

**폭에서는 세 방식이 안 갈린다.** 깊이는 갈린다 — 중앙값이 이 조건에서
