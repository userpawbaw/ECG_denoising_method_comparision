...
- **원본을 대거나, 원본이 없다고 적는다.** B 도 레퍼런스를 갖는다 — 「B 는 레퍼런스를 안 본다」는
  UD-12 **이전** 규약이다. 카드를 못 만드는 환경이면 후보 표 **위에** 「레퍼런스 없음 · 이유 ·
  그래서 무엇에 기대고 있나」를 적는다. `check_records.py` 가 둘 중 하나를 요구한다 (UO-6).
