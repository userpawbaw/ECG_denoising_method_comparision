...
**(4) QRS 직후 준위 이동** `s_err`   <- 새로 넣는다
    ...
    **처음에는 이 지표를 「S 골 오차」라고 불렀는데 그 이름이 틀렸다** —
    정의에 «그 박동의 T-P 준위» 가 끼어 있어서, **S 골이 그대로여도 기준
    준위가 움직이면 숫자가 변한다.** 기준선이 필요 없는 `R−S 봉우리차` 로
    재면 네 방식이 사실상 같다(D1 112.2~113.0 %R). 그러니까 이 지표가 재는
    것은 **QRS 옆의 기준 준위 = ST 구간의 이동**이다. `docs/16` §1.

    이름을 고친 이유는 미용이 아니다. 「S 골이 조금 얕다」는 파형 문제지만
    「ST 준위가 4 %R 밀린다」는 성질이 다르고, **어느 쪽인지에 따라 방식
    선택이 갈린다.** 이름이 틀리면 그 판단을 못 한다 (F-30 과 같은 계열).
"""
...
def write_doc(per_axis: dict, made: list[Path], costs: list[dict]) -> Path:
    L = ["# 13. 위상 왜곡 — 차수 말고 다른 길이 있는가", "",
         ...
         "| **(4) QRS 직후 준위 이동** | ST 구간이 위아래로 밀리는가 | R+80 ms 최저점과 "
         "그 박동 T-P 준위의 거리, 영위상 대비 차 [%R] |",
         "",
         "> **(4) 는 처음에 「S 골 오차」라고 불렀는데 그 이름이 틀렸다** — S 골 자체는\n"
         "> 어느 방식도 안 깎는다(기준선 없는 `R−S 봉우리차` 로 재면 네 방식이 같다).\n"
         "> 움직이는 것은 **QRS 옆의 기준 준위 = ST 구간**이다. `docs/16` §1.",
         "| **(5) T 진폭 오차** | T 파를 깎지 않는가 | R+150~400 ms 최고점 높이의 영위상 대비 차 [%R] |",
         "",
         "**(3)(4) 는 이번에 새로 넣었다.** (1)(2) 만으로는 화면에서 실제로 거슬리는",
         "두 가지 — 박동 사이 준위 어긋남과 QRS 직후 준위 이동 — 을 못 잡는다.",
    ...
    L += ["",
          ...
          "", "## 무엇을 고를 것인가", "",
          "> **이 절의 권고는 `docs/16` 의 감사로 조건이 붙었다.** 아래 표의 근거 중",
          "> 마지막 줄(«선형이다 — 참조를 `FE(clean)` 으로 두므로»)이 문제였다. 그 참조는",
          "> **오프라인 영위상 자신**이고, 블록 영위상은 미리보기를 늘리면 거기로",
          "> **수렴하도록 정의된 방식**이다. 참값이 있는 합성축에서 다시 재면 순위가",
          "> 뒤집힌다(블록 −29.8 대 중앙값 −30.6 dB). 대신 **ST 충실도**에서는 블록이",
          "> 확실히 이긴다(−0.2 대 −2.2 %R, ST 보존 101 대 84 %). `docs/16` §3~4.", "",
